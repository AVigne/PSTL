...
public class ASTIf extends ASTStatement {
	...
	/***
	 * Cas d'un If auquel on donnerait une condition, et deux listes. Non utilisé
	 * 
	 * @param nom
	 * @param valeur
	 * @param c
	...
	public ASTIf(String nom, Object valeur, ASTStatement e) {
		...
		// True -> dans le then pour qu'elle soit éxécutée, dans le else une nouvelle
		// affectation, d'une opération random de valeurs randoms
		if ((Boolean) valeur) {
			thn.add(e);
			els.add(new ASTAffect(e.getType(), e.getNom(),
					(AST) ASTOpBinaire.getRandomOperation(RandomProvider.nextInt(RandomProvider.nbRandom))));
		}
		// False -> dans le else, dans le then, idem que le then
		else {
			els.add(e);
			thn.add(new ASTAffect(e.getType(), e.getNom(),
					(AST) ASTOpBinaire.getRandomOperation(RandomProvider.nextInt(RandomProvider.nbRandom))));
